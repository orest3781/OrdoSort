...
public class FilenameListTests : IDisposable
{
    ...
    /// <summary>An ordinary (non-drive) root with an explicit trailing separator
    /// still trims and relativizes correctly. This does NOT exercise the
    /// drive-letter branch — a temp directory path is never two characters — see
    /// ADriveRootAnchorsAtTheDriveNotTheProcessCurrentDirectory for that case.</summary>
    [Fact]
