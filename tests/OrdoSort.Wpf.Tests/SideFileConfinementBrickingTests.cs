...
/// config.json, never the box-labels bootstrap) was wired into
...
/// full <c>Config.TrySave</c>, which independently attempts the box-labels
/// bootstrap on every call; a box_labels_file whose configured path resolves outside the
...
/// save.
///
/// <see cref="AnOrdinarySettingsSaveStillWritesEverySection"/>
...
public class SideFileConfinementBrickingTests
{
    ...
    [Fact]
    public void AnOrdinarySettingsSaveStillWritesEverySection()
    {
        ...
        Assert.Contains(onDisk.Routes, r => r.Label == "NewRoute");
        Assert.Contains(onDisk.WatchFolders, w => w.Label == "NewWatch");
        Assert.Contains("URGENT", onDisk.AlertTexts);
    }
    ...
    /// persisting the user's Settings edits. Before the fix, a station that had already been warned once
    ...
    /// save would hit the very same confinement refusal with NO dialog at
    /// all, leaving the user believing everything they changed was saved. This fails against the pre-fix ShellViewModel
