...
- [x] **DW-13** · Repo / process — DONE 2026-09-02: moved to `A:\DEV\OrdoSort-samples\sample`, verified 412 files / 23 MB at the destination and the source gone. Was: `docs/sample/` held 412 files / 23 MB of real exports in the working tree (untracked, ignored, never committed — but a QC subagent read a row of one CSV during the 2026-08-21 pass, exactly what the `.gitignore` comment predicts). `A:\DEV\OrdoSort-samples` already exists; moving the folder closes it. *(app-qc §Working-tree note + memory: ordosort-app-qc-2026-08-21)* `(user, 2 sources)`
