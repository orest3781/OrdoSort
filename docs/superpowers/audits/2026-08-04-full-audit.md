...
> **Superseded 2026-08-08.** The parenthetical above — "DPAPI scope `CurrentUser` with null entropy is the correct choice here — do not 'fix' it to `LocalMachine`" — held for a single-station deployment. It no longer holds for this one, because the deployment changed: the owner runs one `config.json` shared over SMB across several stations, not a single machine. That single fact is fatal to DPAPI in **either** scope — `CurrentUser` binds the blob to one Windows account, `LocalMachine` binds it to one PC, and neither travels between stations. Saved passwords are now stored **plaintext, by the owner's deliberate choice**, with the shared folder's own permissions as the security boundary; a per-station passphrase was offered as an alternative and declined. `LocalMachine` was considered and rejected — not primarily because it's less safe (it is: it additionally exposes the blob to every other account on that PC), but because **it does not solve the stated problem at all**: a blob protected under `LocalMachine` on Station A is exactly as unreadable on Station B as one protected under `CurrentUser` on Station A. Do not re-read the old ruling as license to "fix" the scope to `LocalMachine` — that fixes nothing here and adds a same-machine exposure besides. See `docs/superpowers/plans/2026-08-08-portable-saved-passwords.md` and implementation commit `0474784`.
