...
**XamlParseException resource-lookup burst.** After a long sequence of rebuilds, the Wpf test run can fail with a burst of `XamlParseException: Cannot find resource named 'Icon'` / `'PrimaryButton'` (and similar) across unrelated views (`ReadyView`, `ZipToolsWindow`…), sometimes alongside an isolated `BulkRenameViewModelTests` failure. Observed 2026-08-28/29 on this branch, three times, never reproducible on the next run. Cleared every time by `dotnet build-server shutdown` followed by the canonical rebuild. **A second inner message belongs to the same burst** (2026-09-02): `XamlParseException: 'Provide value on 'System.Windows.StaticResourceExtension' threw an exception'` wrapping `System.Xaml.XamlParseException: Found unknown BAML record Unknown` — 1107 of 2236 tests failed across `AccessibleNameTests` and most window suites at once, and the very next run on the SAME binaries passed 2236/2236. Recognise it by the shape: a four-figure failure count spread across unrelated windows, all XamlParseException, none of them assertion failures. Do not start bisecting. A stale `testhost.exe` from an interrupted run can also block the rebuild with `MSB3027` — kill it and rebuild. Not a product defect.
