...
public static class UnzipScenarios
{
    ...
    /// leave Pending.
    ///
    /// Waits on Rows[0].Note, not vm.Summary, and that choice is load-bearing
    ...
    /// including the final `Summary = string.Join(...)` line — to completion
    /// before returning control here. vm.Summary would therefore already be
    /// non-empty on E2EPump.Until's very first (pre-pump) check, exactly the
    /// "filesystem predicate that's already true" trap ScenarioKit.Settle's
    /// own doc comment warns about — except here the trap is a VIEW MODEL
    /// property, not the filesystem. UnzipRow.Apply, by contrast, is only
    /// ever invoked from inside ApplyResult's `_uiContext.Post(...)`, which
    ...
    /// until something pumps. (ZipViewModel.Status doesn't have this split:
    /// its ApplyResult sets Status itself inside the Post-guarded closure, so
    /// Settle(ctx, () => vm.Status) in ZipScenarios is safe as written; Unzip
    /// separates the aggregate Summary from the per-row Apply, and only the
    /// latter is where the marshalling hop actually lives.)</summary>
    ...
    private static void ZipSlip(ScenarioContext ctx)
    {
        ...
        ctx.Check("extraction refused", vm.Rows[0].StatusKind != UnzipRowStatus.Ok,
    ...
    private static void CorruptArchive(ScenarioContext ctx)
    {
        ...
        ctx.Check("reported as invalid", vm.Rows[0].StatusKind != UnzipRowStatus.Ok,
            $"status was {vm.Rows[0].StatusKind}");
