...
- [ ] 🔴 `check.bat` (and `dotnet test`) report success when a test DLL is blocked and NO tests run: on 2026-09-23 Windows Application Control blocked the Release `OrdoSort.Wpf.Tests.dll`, xUnit skipped all 2,364 tests, and check.bat exited 0. Make "no tests ran" a failure (VSTest `RunConfiguration.TreatNoTestsAsError=true`), locally and in CI
