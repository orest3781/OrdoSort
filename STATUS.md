...
| Rebrand (plan: `~/.claude/plans/i-want-you-to-floofy-fiddle.md`) | ✅ Done | 2026-09-24, pushed and **released as v1.7.0** 2026-09-25 (CI, E2E and release runs green; four zips attached; BoxLabels.exe in the zip reports 1.7.0 and its notices carry the font licence). Owner picked concept A (tab dividers), palette A (ink & bronze), Atkinson Hyperlegible Next. 7 schemes → light/dark (paper→light, graphite→dark, other five→auto); new palettes; bundled font; Box Labels Auto/Light/Dark switch; art-as-code generator (`OrdoSort.Smoke brand`); new app icons; redrawn empty-state pictures (Unlock shackle fixed); website; README banner; `docs/brand/BRAND.md`. Checked live: OrdoSort loads "ledger" as Auto and saves it back as "auto", Settings → Light applies at once and saves "light"; new title-bar icons and font in both apps; Box Labels loads "graphite" as Dark, Light applies live, keeps the store path in box-labels-app.json and survives a restart; website served locally in both themes and at phone width |
...
## Next
- [x] Release notes say the colour schemes are down to Light and Dark (Ledger, Microfilm, Manila, Carbon, Blueprint → Auto; Paper → Light, Graphite → Dark): done in the v1.7.0 notes
...
- [ ] Timing-flaky under check.bat load, 2026-09-24: `FolderWatchServiceTests.AFileLandingReachesTheDebounce`, `BulkRenameProbeTests.TypingABurstRunsThePlanOnceNotPerKeystroke` and `BulkRenameViewModelTests.TheRowSelectionSurvivesARebuiltPreview` all timed out at the same second (3 s waits); the rerun passed 1,700/1,700. Same day, `BulkRenameViewModelTests.ReviewTransformParsesTheMedicalFaxNames` threw a NullReferenceException inside its WaitFor condition once; 3/3 alone and the full rerun passed. The BulkRename wait-based tests look racy as a family
