...
- [ ] Dashboard tile groups and tiles announce as "OrdoSort.Wpf.ViewModels.TileGroupViewModel" / "TileViewModel" to screen readers (seen via UI Automation 2026-09-23). AccessibleNameTests only walks registry windows, not the Ready dashboard
- [ ] The inbox scan reports a placeholder like `dev\inbox\.gitkeep` as "1 other file ignored". The owner ruled hidden/dot files out of the set-aside count; decide whether the inbox's ignored-file count follows the same rule
