...
| Rebrand (plan: `~/.claude/plans/i-want-you-to-floofy-fiddle.md`) | 🔄 In progress | Owner picked concept A (tab dividers), palette A (ink & bronze), Atkinson Hyperlegible Next. Built 2026-09-24, local commits, not pushed: 7 schemes → light/dark (paper→light, graphite→dark, other five→auto); new palettes; bundled font; Box Labels Auto/Light/Dark switch; art-as-code generator (`OrdoSort.Smoke brand`); new app icons; redrawn empty-state pictures (Unlock shackle fixed); website; README banner; `docs/brand/BRAND.md`. Left: live check of both apps, release notes for the next version (5 schemes removed, those users reset to Auto) |
