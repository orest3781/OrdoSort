...
public sealed class SettingsViewModel : ObservableObject, IDisposable
{
    ...
    // below (Inbox/Deferred/NamesFile/HistoryDb/BoxLabelsFile) probe
    ...
    // ---- box-labels.json, the one section still in its own file — Settings
    // edits the path, not the section's contents
    ...
    /// <summary>Browse... for the box_labels_file key, then refuse the result up
    ...
    public List<string> HardErrors()
    {
        ...
        // QC-08: the box-labels field's own DebouncedProbe note sees only
        // its own value, not whether it lands on config.json itself.
        // HardErrors runs before OK is accepted, so the user is told here
        // instead of finding out from a refused Save. Reuses the same
        // Config.TryFindSideFileCollision check Save/TrySave run, after the
        // same blank-means-default substitution BuildEditedConfig applies.
        ...
        // use): there is no base directory to resolve a relative path against.
        if (_cfgPath is { } cfgPathForCollision)
        {
            var boxFile = BoxLabelsFile.Trim().Length == 0
                ? Config.DefaultBoxLabelsFile : BoxLabelsFile.Trim();
            if (Config.TryFindSideFileCollision(cfgPathForCollision, boxFile, out _))
                errors.Add("\"Box labels\" is set to the settings file itself — point it at a " +
                           "file of its own.");
        }
    ...
    private Config BuildEditedConfig()
    {
        ...
        // own unknown keys, not config.json's), so the clone above drops it
        // back to empty. Carry the original's copy through by hand; the
        // box-labels bootstrap write would otherwise lose hand-added keys.
